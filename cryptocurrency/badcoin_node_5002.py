...
class Blockchain:
    ...
    def __init__(self):
        ...
        self.chain = []         
        #creating empty list where transaction has to be stored before the block(which has to store these transactions) is mined
        self.transactions = []  
    ...
    def create_block(self,proof,previous_hash):
        block = {'index': len(self.chain)+1,   #index starts from 1
                 'timestamp':str(datetime.datetime.now()),
                 'proof':proof, 
                 'previous_hash': previous_hash,
                 'transactions': self.transactions #all these Transactions get added to the block when the block is mined
                  }
    ...
    def is_chain_valid(self,chain): #checks two conditions - 1)'hash' of previous block = 'previous hash' of current block
                                                              #  2) hash generated for current block much lie in the targeted region having '0000' in the beggining
    ...
    def replace_chain(self):  #this function replaces the node having short chain length with the node having greater chain length and returns whether chain repacement is done or not
        network = self.nodes  
        longest_chain = None
...
def min_block():
    ...
    blockchain.add_transaction(sender = node_address , receiver = 'arnab',amount = 1) #adding the transaction to the object blockchain that we created
